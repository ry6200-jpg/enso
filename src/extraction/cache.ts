...
export class ExtractionCache {
  ...
  get(key: CacheKey): ExtractionStructure | undefined {
    const row = this.db
      .prepare(
    ...
    return row ? (JSON.parse(row.output) as ExtractionStructure) : undefined;
  }

  put(key: CacheKey, output: ExtractionStructure): void {
    this.db
      .prepare(
