...
describe("createCachedRouter (EN-056, real-provider reuse)", () => {
  it("misses then hits for the same request text, never calling the underlying router twice", async () => {
    ...
    const fakeRouter: ExtractionRouter = {
      extract: async () => {
        calls++;
        return { provider: "openai", model: "gpt-5.6-terra", taxonomy: { entities: [], statedFeelings: [], episodeMarkers: [], structuralAtoms: [], socialBonds: [], attributes: [] }, usage: { inputTokens: 5, outputTokens: 5 } };
      }
    };
  ...
  it("misses separately for different extractor_version, even with identical text", async () => {
    const fakeRouter: ExtractionRouter = {
      extract: async () => ({ provider: "gemini", model: "gemini-3.7-flash", taxonomy: { entities: [], statedFeelings: [], episodeMarkers: [], structuralAtoms: [], socialBonds: [], attributes: [] }, usage: { inputTokens: 1, outputTokens: 1 } })
    };
