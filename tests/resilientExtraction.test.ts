...
describe("extractMessageWithResilience (EN-059/060)", () => {
  it("on success, appends extraction_completed with taxonomy and classifier decision", async () => {
    ...
    const router: ExtractionRouter = {
      extract: async () => ({
        provider: "openai",
        model: "gpt-5.6-terra",
        taxonomy: { entities: [{ name: "Sarah", type: "person" }], statedFeelings: [], episodeMarkers: [] },
  ...
  it("skips the LLM call entirely for non-personal content — no provider/model recorded", async () => {
    ...
    const router: ExtractionRouter = {
      extract: async () => {
        called = true;
        return { provider: "openai", model: "gpt-5.6-terra", taxonomy: { entities: [], statedFeelings: [], episodeMarkers: [] }, usage: { inputTokens: 0, outputTokens: 0 } };
      }
    };
  ...
  it("retries on availability errors and succeeds, recording status completed", async () => {
    ...
    const router: ExtractionRouter = {
      extract: async () => {
        ...
        return { provider: "gemini", model: "gemini-3.7-flash", taxonomy: { entities: [], statedFeelings: [], episodeMarkers: [] }, usage: { inputTokens: 1, outputTokens: 1 } };
      }
    };
  ...
  it("a failed extraction can be retried to success via retryFailedExtraction (EN-059's retry entry point)", async () => {
    ...
    const router: ExtractionRouter = {
      extract: async () => {
        ...
        return { provider: "openai", model: "gpt-5.6-terra", taxonomy: { entities: [], statedFeelings: [], episodeMarkers: [] }, usage: { inputTokens: 1, outputTokens: 1 } };
      }
    };
