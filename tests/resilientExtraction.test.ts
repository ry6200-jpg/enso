...
describe("extractMessageWithResilience (EN-059/060)", () => {
  it("on success, appends extraction_completed with the FULL taxonomy (entities, atoms, bonds, attributes) and classifier decision", async () => {
    ...
    const router: ExtractionRouter = {
      extract: async () => ({
        ...
        taxonomy: {
          ...
          statedFeelings: [],
          episodeMarkers: [],
          structuralAtoms: [{ type: "sibling_of", fromName: "me", toName: "Sarah", action: "assert", explicitlyNewPerson: false, fromNameIsRoleWord: false, toNameIsRoleWord: false }],
          socialBonds: [{ type: "friend", fromName: "me", toName: "Sarah", qualifier: null, basis: "stated", action: "open", explicitlyNewPerson: false, fromNameIsRoleWord: false, toNameIsRoleWord: false }],
    ...
    expect(payload.structuralAtoms).toEqual([{ type: "sibling_of", fromName: "me", toName: "Sarah", action: "assert", explicitlyNewPerson: false, fromNameIsRoleWord: false, toNameIsRoleWord: false }]);
    expect(payload.socialBonds).toEqual([{ type: "friend", fromName: "me", toName: "Sarah", qualifier: null, basis: "stated", action: "open", explicitlyNewPerson: false, fromNameIsRoleWord: false, toNameIsRoleWord: false }]);
