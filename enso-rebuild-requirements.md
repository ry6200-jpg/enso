...
**EN-030. Entity establishment (circle-back).** When a new person is mentioned, Enso tactfully works toward a small set of key facts through natural conversation — digression-tolerant, never a blocking gate, never framed as setup. Implemented as a **deterministic gate**, not a passive prompt instruction (see EN-072). Rules, all settled: one entity per circle-back; never two consecutive turns; minimum ~5-turn gap between a first and any later attempt; hard cap of 2 attempts per entity, ever, then permanently dormant regardless of anything else — the user can always volunteer the relationship themselves. An attempt is recorded only after verifying the ask actually appeared in the reply. Phrasing must vary; near-verbatim repetition across turns is a defect. **Priority (EN-041, "the user is the most important entity"):** a self-entity priority — currently the primary user's own birthdate — outranks any third-party circle-back candidate outright; third-party candidates are not even offered to the gate while a higher-priority self-fact remains unestablished, never merely weighed against them on equal footing.
