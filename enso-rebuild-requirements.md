...
**EN-030. Entity establishment (circle-back).** When a new person is mentioned, Enso tactfully works toward a small set of key facts through natural conversation — digression-tolerant, never a blocking gate, never framed as setup. Implemented as a **deterministic gate**, not a passive prompt instruction (see EN-072). Rules, all settled: one entity per circle-back; never two consecutive turns; minimum ~5-turn gap between attempts; hard cap of 2 attempts per entity, then dormant, permanently — the user can always volunteer the relationship themselves. An attempt is recorded only after verifying the ask actually appeared in the reply. Phrasing must vary; near-verbatim repetition across turns is a defect.

**Recency scope — Option B (decided Phase 7 Part 0, resolving a real deadlock caught while porting this gate to the new architecture):** the "only recently-mentioned entities are proactively raised" rule applies to an entity's **first** attempt only. A still-unestablished entity that already had one attempt **waives** the recency window on its retry — unresolvedness is itself the standing reason to ask again, not how long ago the person first came up. (Rationale: with the old repo's original 5-turn recency window and 5-turn cooldown sharing the same value, a second attempt on the same entity was structurally near-unreachable — by the time cooldown cleared, the entity's introduction had always already aged out of the window. Verified live during Phase 6 before the fix: 0/N firings ever reached a second attempt.) Priority when both a fresh (first-attempt, recency-eligible) candidate and a retry candidate are available the same turn: the fresh candidate always wins — retries only fill lulls, never interrupt the present a fresh mention creates. The gate passes the attempt number (1 or 2) and a bucketed mention-age label to the persona layer so retry phrasing can bridge the gap explicitly ("the other day you mentioned Marcus — where does he fit?") — this is Enso returning to its own open thread, not counting the owner's repetitions back at them (that remains a strictly separate, untouched rule, EN-043).
